...
export const Chat = ({ className = '', chatId = null, onChatCreated }: ChatProps) => {
  ...
  const fetchAiModels = async () => {
    try {
      ...
      if (initialModelId && options.some(option => option.value === initialModelId)) {
  ...
  const onStreamEvent = (event: StreamEvent) => {
    switch (event.type) {
      ...
      case 'message_content':
        handleMessageContent(assistantMessageIdRef.current, event.content?.text);
        break;
